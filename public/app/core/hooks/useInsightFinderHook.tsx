...
export default function useInsightFinderHook() {
  ...
  const { components } = usePluginComponents<InsightFinderNavbarActionProps>({
    extensionPointId: 'grafana/bmc/insight-finder/navbar-actions',
  });
  ...
  const InsightFinderComponent = useCallback(() => {
    ...
    return <InsightFinderComp onPanelAdd={onPanelAdd} />;
...
const canAccessInsightFinder = (): boolean =>
  getGrafanaFeatureStatus(FEATURE_CONST.ENABLE_INSIGHT_FINDER);
