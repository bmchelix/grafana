...
import { DashFolderLinkRegexp } from '../internationalization/constants';
...
function translateNav(navTree: NavModelItem[]): NavModelItem[] {
  return navTree.map((navItem) => {
    ...
    if (navItem.pluginId === 'bmc-insightfinder-app') {
      ...
      } else if (navItem.text === 'Custom Data Integrator') {
        navItem.id = 'insight-finder/custom-data-integrator';
      }
    }
