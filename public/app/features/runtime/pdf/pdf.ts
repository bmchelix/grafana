...
import { CreateFromHTMLOptions, CreateFromCSVOptions, FontOptions, TemplateOptions, ThemeOptions } from './types';
import { parseCSV, parseCsvMeta, rowsLimitNote, blendWithTheme } from './utils';
...
class PDFGenerator {
  ...
  /**
   * Registers a font with the PDF document.
   *
   * @param {FontOptions} options - The options for the font to be registered.
   * @param {string} options.fontName - The name of the font.
   * @param {string} options.fileName - The name of the file containing the font.
   * @param {string} options.fileContent - The content of the font file.
   * @param {string} [options.fontStyle='normal'] - The style of the font (e.g., 'normal', 'bold', 'italic').
   */
  public registerFont({ fontName, fileName, fileContent, fontStyle = 'normal' }: FontOptions) {
    this.doc.addFileToVFS(fileName, fileContent);
    this.doc.addFont(fileName, fontName, fontStyle);
  }
  ...
  public createHeader = (theme: ThemeOptions, recordsCount = 0): PageHook => {
    ...
    return (data) => {
      ...
      this.doc.setFontSize(8).setFont(PDFGenerator.fontFamily, 'bold');
      if (!hideDashboardPath) {
        this.doc.text(this.options.dashboardPath, margin, 20);
      }
      ...
      if (template.timeRange.from) {
        ...
        this.doc.setFontSize(8).setFont(PDFGenerator.fontFamily, 'normal');
      ...
      this.doc.setFontSize(12).setFont(PDFGenerator.fontFamily, 'bold');
      let pageWidth = this.doc.internal.pageSize.width || this.doc.internal.pageSize.getWidth();
      let text = this.doc.splitTextToSize(template.reportName!, pageWidth - 35, {});
      this.doc.text(text, margin, 50);

      if (!!template.reportDescription) {
        this.doc.setFontSize(10).setFont(PDFGenerator.fontFamily, 'normal');
      ...
      if (data.pageNumber === 1) {
        this.doc.setFontSize(10).setFont(PDFGenerator.fontFamily, 'bold');
  ...
  public createFooter = (theme: ThemeOptions): PageHook => {
    ...
    return (_) => {
      this.doc
        .setFillColor(theme.headerSectionColor)
        .setTextColor(theme.headerTextColor)
        .setFontSize(8);
      ...
      if (!this.options.hideDashboardPath) {
        this.doc.setFontSize(8).setFont(PDFGenerator.fontFamily, 'bolditalic');
        this.doc.text('Generated on:', 15, this.doc.internal.pageSize.getHeight() - 15);
        this.doc.setFont(PDFGenerator.fontFamily, 'italic');
      ...
      this.doc
        .setFont(PDFGenerator.fontFamily, 'bolditalic')
        .text(template.footerText!, xStartingPoint, this.doc.internal.pageSize.getHeight() - 15)
        .link(xStartingPoint, this.doc.internal.pageSize.getHeight() - 30, this.doc.getTextWidth(template.footerText!), 15, {
  ...
  public createCell(theme: ThemeOptions): CellHook {
    return (data: any) => {
      ...
      if (data.cell.section === 'body' && data.cell.meta) {
        ...
        if (columnMeta.type && columnMeta.type === 'b') {
          ...
          this.doc.setFont(fonts.barcode.fontName, fonts.barcode.fontStyle);
        } else {
  ...
  public createFromCSV = async ({ csvContent }: CreateFromCSVOptions): Promise<void> => {
    ...
    const totalCustomWidth = headersMeta.reduce((sum, headerMeta) => {
   ...
   */
  public createFromHTML = async ({ selector, theme = 'grid', useCss = false, firstColumnWidth }: CreateFromHTMLOptions): Promise<void> => {
