import { FONT_REGISTRY, type Script } from 'adereporting-node-sdk';
...
import { getMultilingualFont, loadAndRegisterBarcode } from './fontUtils';
import { CreateFromCSVOptions, CreateFromHTMLOptions, TemplateOptions, ThemeOptions } from './types';
...
class PDFGenerator {
    ...
    return (data: any) => {
      ...
      if (data.cell.section === 'head') {
        ...
        if (columnMeta.type && columnMeta.type === 'b') {
          ...
          const barcodeMeta = FONT_REGISTRY['barcode']!;
          this.doc.setFont(barcodeMeta.fontName, barcodeMeta.fontStyle);
        } else {
          this.doc.setFontSize(10).setFont(this.fontFamily, 'normal');
        }
      } else {
  ...
  public createFromCSV = async ({ csvContent }: CreateFromCSVOptions): Promise<void> => {
    ...
    if (barcodeColumnsLength) {
      await loadAndRegisterBarcode(this.doc);
    }
