...
import { blendWithTheme, parseCSV, parseCsvMeta, rowsLimitNote } from './utils';
...
class PDFGenerator {
  ...
  public createFooter = (theme: ThemeOptions): PageHook => {
    ...
    return (_) => {
      ...
      this.doc
        .setFont(this.fontFamily, this.fontFamily === PDFGenerator.defaultFont ? 'bolditalic' : 'bold')
  ...
  private async handleMultilingualFonts(headersMeta: any[]): Promise<string | null> {
    ...
    return await getMultilingualFont(
      this.doc,
      [brand.reportName, brand.reportDescription, table.panelTitle, dashboardPath],
      scriptFromCSV
    );
  }
}
