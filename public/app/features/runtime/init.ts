...
import { PanelData, RawTimeRange, PanelModel as IPanelModel, toCSV} from '@grafana/data';
...
export function initWindowRuntime() {
  window.grafanaRuntime = {
    ...
    getPanels: () => {
      ...
      return getFeatureStatus('bhd-localization')
        ? d.panels.map((p) => {
        ...
        autoTable(doc, {
          ...
          willDrawPage: (data: any) => {
            ...
            if(!!template.description){
              doc.setFontSize(7).setFont(fontFamily, 'italic');
    ...
    makePDF: async (panelId: number, fileName: string, opt: RuntimePDFOptions): Promise<any> => {
      ...
      const doc = new PDF({
        ...
        table: {
          panelTitle: panel.getDisplayTitle(),
        ...
        brand: {
          ...
          generatedAt: opt.generatedAt || moment().format(config.bootData.settings.dateFormats?.fullDate || 'YYYY-MM-DD HH:mm:ss Z'),
          timeRange: {
            to: opt.timeRange.to,
      ...
      doc.registerFont(fonts.barcode);
      ...
      if (panel.type === 'table' || panel.type === 'bmc-table-panel') {
        ...
        let csvString = ''
        try {
          const data = await firstValueFrom(queryRunner.getData({ withFieldConfig: true, withTransforms: true }));
          const series = data?.series ?? [];
          csvString = toCSV(series, { useExcelHeader: false, enableOverrides: true });
          if (series.length > 0) {
          }
        } catch (err) {
      ...
      if (panel.type === 'bmc-ade-cross-tab') {
        // JSPDF-Autotable understands the HTMLTableElement instance to generate the PDF so redirecting
        // renderrer to open the view cross tab panel and finding the instance using the css selector.
        const selector: any = document.querySelector<HTMLTableElement>(`[data-panelid="${panelId}"] table`);
      ...
      if (panel.type === 'bmc-record-details') {
        let elements = document.querySelectorAll(`[data-panelid="${panelId}"] .responsive_record_details`);
        ...
        await doc.createFromHTML({
          ...
          theme: 'plain',
          useCss: false,
          firstColumnWidth: isColumn ? undefined : panel.options.widthPercent || 25
        })
        table.remove();
      }
