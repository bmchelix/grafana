...
declare global {
  interface Window {
    grafanaRuntime?: {
      ...
      openInspect: (panelId: string, tab?: string) => boolean;
    };
  }
}
...
export function initWindowRuntime() {
  window.grafanaRuntime = {
    ...
    openInspect: (panelId: string, tab?: string): boolean => {
      const d = getDashboardSrv().getCurrent();
      // @ts-expect-error
      return d?.openInspect?.(panelId, tab) ?? false;
    },
     ...
     *   Set `template.isRtl: true` or `template.direction: 'rtl'` to mirror header/footer layout for cross-tab PDFs.
     *   Optional `template.locale` or `template.language` starting with `ar` selects Arabic header labels (same as renderer PDF template).
     */
    generatePdf: async (fileName: string, template: any) => {
      const urlParams = new URLSearchParams(window.location.search);
      let panelId = urlParams.get('viewPanel') || '0';
      panelId = config.bootData.settings.featureToggles.dashboardScene && !isNaN(Number(panelId)) ? `panel-${panelId}` : panelId
      ...
      if (panel?.type === 'bmc-ade-cross-tab') {
        ...
        const isArabicPdf = template.isArabicRTL;
        ...
        const fontFamily = await getMultilingualFont(
          doc,
          [
            selector?.innerText || '',
            template.reportName || '',
        ...
        const rangeTitleAr = 'نطاق التاريخ والوقت';
        const toWordAr = 'إلى';
        const generatedLabelAr = 'تاريخ الإنشاء';
        const rangeTitleDraw = isArabicPdf ? rangeTitleAr : 'Data time range: ';
        const toWordDraw = isArabicPdf ? toWordAr : 'to';
        const generatedLabelDraw = isArabicPdf ? generatedLabelAr : 'Generated on:';
        ...
        autoTable(doc, {
          ...
          styles: {
            ...
            ...(currentFontFamily !== defaultFontFamily ? { font: currentFontFamily } : {}),
          },
          headStyles: {
          ...
          willDrawPage: (data: any) => {
            ...
            if (template.from) {
              if (!isArabicPdf) {
                ...
                doc.setFontSize(8).setFont(currentFontFamily, 'bold');
                const rangeLabelWidth = doc.getTextWidth(rangeTitleDraw + template.from) + 15;
                doc.text(rangeTitleDraw, doc.internal.pageSize.getWidth() - rangeLabelWidth - 5, 20);
                const toLabelWidth = doc.getTextWidth(toWordDraw + template.to) + 15;
                doc.text(toWordDraw, doc.internal.pageSize.getWidth() - toLabelWidth - 5, 30);
                doc.setFontSize(8).setFont(currentFontFamily, 'normal');
                // Below 65 is calculated by doc.getTextWidth(rangeTitleDraw) + some margin
                doc.text(
                  template.from,
                  doc.internal.pageSize.getWidth() - rangeLabelWidth + doc.getTextWidth(rangeTitleDraw) + 5,
                  20
                );
                // Below 10 is calculated by doc.getTextWidth(toWordDraw) + some margin
                doc.text(
                  template.to,
                  doc.internal.pageSize.getWidth() - toLabelWidth + (isArabicPdf ? doc.getTextWidth(toWordDraw) : 10),
                  30
                );
              }
            }
            ...
            if (!!template.description) {
              ...
              const descMaxWidth = pageWidth - 35;
              let description = doc.splitTextToSize(template.description, descMaxWidth, {});
              doc.text(description, margin, 65);
            }
            // Displaying the panel title only on the first page of the pdf for the table panel.
            let resolvedPanelTitle = config.bootData.settings.featureToggles.dashboardScene
              ? panel.title
              : panel.getDisplayTitle();
            if (data.pageNumber === 1) {
              doc.setFontSize(10);
              doc.text(resolvedPanelTitle, margin, 95);
            }
          ...
          didDrawPage: (data: any) => {
            ...
            if (!hideDashboardPath) {
              if (!isArabicPdf) {
                ...
                doc.setFontSize(8).setFont(currentFontFamily, 'bold');
                doc.text(generatedLabelDraw, margin, doc.internal.pageSize.getHeight() - 15);
                doc.setFont(currentFontFamily, 'normal');
                doc.text(
                  template.generatedAt,
                  margin + doc.getTextWidth(generatedLabelDraw) + 5,
    ...
    makePDF: async (panelId: number | string, fileName: string, opt: RuntimePDFOptions): Promise<any> => {
      ...
      if (panel.type === 'bmc-ade-cross-tab') {
        ...
        const selector = config.bootData.settings.featureToggles.dashboardScene
          ? document.querySelector<HTMLTableElement>(`[data-viz-panel-pathid="${panelId}"] table`)
      ...
      if (panel.type === 'bmc-record-details') {
        let elements = config.bootData.settings.featureToggles.dashboardScene
          ? document.querySelectorAll(`[data-viz-panel-pathid="${panelId}"] .responsive_record_details`)
