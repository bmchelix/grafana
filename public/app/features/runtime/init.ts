...
declare global {
  interface Window {
    grafanaRuntime?: {
      ...
      panelHasData: (panelId: number) => Promise<boolean>;
    };
  }
}
  ...
  window.grafanaRuntime = {
    ...
    makePDF: async (panelId: number, fileName: string, opt: RuntimePDFOptions): Promise<any> => {
      ...
      if (panel.type === 'bmc-ade-cross-tab') {
        // JSPDF-Autotable understands the HTMLTableElement instance to generate the PDF so redirecting
        // renderrer to open the view cross tab panel and finding the instance using the css selector.
        const selector = document.querySelector<HTMLTableElement>(`[data-panelid="${panelId}"] table`);
    ...
    panelHasData: async (panelId: number): Promise<boolean> => {
      const dashboard = getDashboardSrv().getCurrent();
      if (!dashboard) {
        return false;
      }
      const panelIdNum = Number(panelId);
      const panelInfo = dashboard.panels.find((p) => p.id === panelIdNum);
      const panelData: Record<number, PanelData | undefined> = {};
      if (!panelInfo) {
        return false;
      }
      const queryRunner = panelInfo.getQueryRunner();
      // Handling of Text and Dynamic Text panels as queryRunner.getData promise stays as pending
      panelData[panelIdNum] = queryRunner.getLastResult();
      if (!panelData[panelIdNum]) {
        return false;
      }
      try {
        const data = await firstValueFrom(queryRunner.getData({ withFieldConfig: true, withTransforms: true }));
