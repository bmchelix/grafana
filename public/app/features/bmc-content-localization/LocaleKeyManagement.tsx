...
import { DEFAULT_LANGUAGE as ENGLISH_US, t, Trans } from '@grafana/i18n';
import { Button, Drawer, Field, Icon, InlineLabel, Input, Select, Stack, useStyles2, useTheme2 } from '@grafana/ui';
import appEvents from 'app/core/app_events';

import { DashboardModel } from '../dashboard/state/DashboardModel';

import { DashboardLocale, initializeDashboardLocale, LanguageCode, LanguageOptions, syncLocaleKeys } from './types';
...
export const LocaleKeyManagement: FC<Props> = ({
  ...
  const ensureKeysInLocale = (targetLocale: LanguageCode) => {
    const { locales: syncedLocales, hasChanges } = syncLocaleKeys(dashLocales, defaultKey as keyof DashboardLocale, targetLocale);
    
    if (hasChanges) {
      dashboard.updateLocalesChanges?.(syncedLocales);
    }
  };
  
  const handleLanguageChange = (value: LanguageCode) => {
    ensureKeysInLocale(value);
    setLanguage(value);
  };
                ...
                <tr key={'dash-name'}>
                  ...
                  <td className={styles.td}>
                    <Icon
                      ...
                      name="trash-alt"
                      title={t('bmc.content-localization.locale-key.delete', 'Delete')}
                    />
                  </td>
                </tr>
              ) : null}
