...
export function StoredNotifications() {
  ...
  const clearSelectedNotifications = () => {
    ...
    if (dismissedCount > 0) {
      const msg =
        dismissedCount === 1
          ? t('notifications.live.dismissed.single', 'Dismissed 1 notification')
          : t('notifications.live.dismissed.multiple', 'Dismissed {{count}} notifications', { count: dismissedCount });
      setLiveMessage('');
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.topRow}>
        <Checkbox
          ...
          aria-label={t('notifications.stored-notifications.select-all', 'Select all Notifications')}
          //BMC Accessibility Change End
