...
export function QueryVariableEditorForm({
  ...
  const RenderVariableCachingToggle = ({ enableVariableCachingToggle }: RenderVariableCachingToggleProps) => {
    ...
    return (
      <InlineFieldRow style={{ marginBottom: '10px', flexDirection: 'column' }}>
        <InlineField
          label={t('bmc.variables.query-editor.variable-caching', 'Enable variable caching')}
          style={{ marginBottom: 0 }}
        ...
        >
          ...
          {!getFeatureStatus(FEATURE_CONST.BHD_ENABLE_DEPENDANT_VAR_CACHING) && (
            <Trans i18nKey="bmc.variables.variable-caching.dependant-warning">
              {' '}
              Caching is not allowed for queries that are dependant on any other variable.
            </Trans>
          )}
        </span>
        <Modal
          isOpen={modalStatus}
          title={t('bmc.variables.variable-caching.modal-close-title', 'Variable caching')}
          onDismiss={() => {
            setModalStatus(false);
          }}
          icon="exclamation-triangle"
        ...
        >
          <h5>
            {!bmcVarCache ? (
              <Trans i18nKey="bmc.variables.variable-caching.modal-enable-confirmation">
                Do you want to enable caching for the variable?
              </Trans>
            ) : (
              <Trans i18nKey="bmc.variables.variable-caching.modal-disable-confirmation">
          ...
          {!bmcVarCache && (
            <div>
              <Trans i18nKey="bmc.variables.variable-caching.modal-enable-additional-text-1">
