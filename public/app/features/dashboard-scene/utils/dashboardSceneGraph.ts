...
import { behaviors, sceneGraph, SceneGridRow, SceneObject, VizPanel } from '@grafana/scenes';
...
export function updateCurrentLocales(locales: DashboardLocale): { currentLocales: DashboardLocale } {
  ...
  if (!!locales) {
    ...
    let globalLocales = {};
    try {
      const gL = store.get('globalLocales');
      if (gL) {
...
export function getDeflatedLayoutChildren(item: SceneObject, index: number, heightMap: { [key: string]: number }) {
  if (item instanceof SceneGridRow) {
    ...
    const tmpArr = [];
    const allPanels = [item.state.body, ...item.state.repeatedPanels];
    const totalPanels = allPanels.length;
    const maxPerRow = Math.min(totalPanels, item.getMaxPerRow());
    const itemHeight = item.state.itemHeight ?? 10;

    for (let i = 0; i < totalPanels; i++) {
      const panel = allPanels[i];
      const tmpItem = item.clone() as DashboardGridItem;
      let width = tmpItem.state.width ?? 24,
        height = itemHeight,
        x = 0;

      if (tmpItem.state.repeatDirection === 'h') {
        width = 24 / maxPerRow;
        x = (i % maxPerRow) * width;
      }
      ...
      tmpItem.setState({
        x,
        key: `${tmpItem.state.key}-${panel.state.key}`,
        width,
        height: height + (heightMap[panel.state.key!] ?? 0),
      ...
      tmpItem.activate();
      tmpArr.push(tmpItem);
      delete heightMap[panel.state.key!];
    }
    return tmpArr;
  } else {
