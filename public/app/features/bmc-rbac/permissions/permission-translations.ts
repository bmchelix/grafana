...
export function getPermissionsDisplayName(name: string) {
  switch (name) {
    ...
    case 'insightfinder.deepanalysis:access':
      return t('bmc.rbac.permissions.insightfinder.deep-analysis-access', 'Deep Analysis');
...
export function getPermissionsDesc(name: string) {
  switch (name) {
    ...
    case 'insightfinder.deepanalysis:access':
      return t(
        'bmc.rbac.permissions.insightfinder.deep-analysis-access-desc',
        'AI-driven root cause and outlier detection engine. Deep analysis needs to be configured with HelixGPT Agent Studio version 26.3.00 or above'
      );
    case 'reports.dynamic.recipients:access':
      return t(
