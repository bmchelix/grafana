...
import { pageChanged, rolesFetchBegin, rolesFetchEnd, rolesLoaded, searchQueryChanged } from './reducers';
...
export async function loadRoleDetails(roleId: number): Promise<BMCRoleDetailsResponse | undefined> {
  if (!contextSrv.hasPermission(AccessControlAction.ActionTeamsRead)) {
    return;
  }
