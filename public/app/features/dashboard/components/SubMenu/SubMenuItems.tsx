...
import { GrafanaTheme2, QueryVariableModel, TypedVariableModel, VariableHide } from '@grafana/data';
import { selectors } from '@grafana/e2e-selectors';
import { getDataSourceSrv } from '@grafana/runtime';
import { useStyles2 } from '@grafana/ui';
import { getVariableQueryRunner } from 'app/features/variables/query/VariableQueryRunner';
import { deleteVariableCache } from 'app/features/variables/utils';
...
export const SubMenuItems = ({ variables, readOnly }: Props) => {
  ...
  const handleRefresh = async (variable: QueryVariableModel) => {
    try {
      ...
      if (!(await deleteVariableCache(queryVar, dashboardUID, false))) {
