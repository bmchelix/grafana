...
import { EnclosureMode, NewlineMode } from '@grafana/data/internal';
import { t, Trans } from '@grafana/i18n';
import { locationService } from '@grafana/runtime';
import { sceneGraph } from '@grafana/scenes';
import { Button, Icon, InlineSwitch, RadioButtonGroup, Select, Spinner, Tooltip } from '@grafana/ui';
...
const getLayoutOptions = (): Array<SelectableValue<PDFLayout>> => [
  { label: t('bmc.export.grid', 'Grid'), value: PDFLayout.GRID },
  { label: t('bmc.export.simple', 'Simple'), value: PDFLayout.SIMPLE },
];
const getOrientationOptions = (): Array<SelectableValue<PDFOrientation>> => [
...
const ExportUtility: React.FC<Props> = (props) => {
  ...
  const [state, setState] = React.useState<State>({
    selectedLayout: getLayoutOptions()[0],
    selectedOrientation: getOrientationOptions()[0],
  ...
  const getVariables = (): string => {
    ...
    return search
      ...
      .split('&')
      .filter((e: string) => e.includes('var'))
      .join('&');
  };
  ...
  const onOrientationChange = (value: PDFOrientation) => {
    const selected = getOrientationOptions().find((e) => e.value === value);
    if (selected) {
  ...
  const onLayoutChange = (value: PDFLayout) => {
    const selected = getLayoutOptions().find((e) => e.value === value);
    if (selected) {
          ...
          <div className="gf-form">
            ...
            <div className={classNameWrapper}>
              <Select
                ...
                aria-label={t('bmc.export.select-report-type-aria', 'Select report type')}
                //Accessibility Change End
                width={selectWidth}
                options={reportTypeOptions}
                defaultValue={reportTypeOptions[0]}
                value={state.selectedReportType}
                placeholder={t('bmc.export.select-report-type-placeholder', 'Select a report type')}
            ...
            <div className="gf-form">
              <label className="gf-form-label width-8" htmlFor="layout">
                ...
                {config.featureToggles.rtlSupport === true &&
                  (config.bootData.user.language || '').split('-')[0].toLowerCase() === 'ar' && (
                    <Tooltip
                      content={t('bmc.export.rtl-grid-only', 'Right-to-Left layout is only available in Grid view')}
                      placement="top"
                    >
                      <Icon name="info-circle" style={{ marginLeft: 4, cursor: 'pointer' }} />
                    </Tooltip>
                  )}
              </label>
              <div className={classNameWrapper}>
