...
export function getFeatureStatus(featureName: string) {
  ...
  const KeyEnabledFeatures = store.get(Key_Enabled_Features);
...
export function loadFeatures(tenantFeature: TenantFeatureDTO[] | null) {
  ...
  store.set(Key_Enabled_Features, JSON.stringify(enabledFeatures));
}
...
export async function loadGrafanaFeatures() {
  ...
  store.set(Key_Grafana_Enabled_Feature, JSON.stringify(enabledFeatures));
  store.set(Key_Features_List, JSON.stringify(featureMap));
  return;
}

export function getGrafanaFeatureStatus(featureName: string) {
  const enabledFeatures = store.get(Key_Grafana_Enabled_Feature);
...
export function getGrafanaFeaturesList() {
  const featuresList = store.get(Key_Features_List);
  if (featuresList) {
    try {
...
export const FEATURE_CONST = {
  ...
  ENABLE_INSIGHT_FINDER: 'Enable Insight Finder', // DRJ71-20115
  RTL_SUPPORT: 'Right-to-Left Interface',
};
...
const getLocalizedFeatures = (cur: string) => {
  switch (`${cur.toLowerCase().replace(/\s+/g, '-')}`) {
    ...
    case 'enable-insight-finder':
      return t('bmc.features-list.enable-insight-finder', 'Enable Insight Finder');
    case 'right-to-left-interface':
      return t('bmc.features-list.rtl-support', 'Right-to-Left Interface');
    default:
      return cur;
  }
};
