  ...
  */
  ...
  useEffect(() => {
    ...
    const getViewList = async () => {
      ...
      setViewList(
        Array.isArray(data)
          ? data.filter((i) => !i.deleted).map((item) => ({ label: item.name, value: item.id }))
          : []
      );
    };
    ...
    <>
      ...
      {inputs.vqbViews &&
        inputs.vqbViews.map((input: ViewInput, index: number) => {
          ...
          return (
            <Field
              label={t('bmcgrafana.import.view-label', 'View: {{index}}', { index: index + 1 })}
              description={input.description}
              key={fieldView}
      ...
      <Stack>
        ...
        <Button type="reset" variant="secondary" onClick={onCancel}>
          {/* BMC code: next conditional tag */}
          {isMultiple ? (
            <Trans i18nKey="bmc.common.delete">Delete</Trans>
          ) : (
