...
import { config, getBackendSrv, getDataSourceSrv, isFetchError } from '@grafana/runtime';
import {
  AnnotationQueryKind,
...
export const exportDashboards = async (req: any, callback?: () => string[]) => {
  ...
  for (const uid of uids) {
    try {
      ...
      // Instead of parsing `content-disposition`, rely on the backend-provided `X-Export-Filename` header.
      // The value is URI-encoded, so decode it when present and fall back to a UID-based filename if missing.
      const encoded = res.headers.get('X-Export-Filename');
      const fileName = encoded ? decodeURIComponent(encoded) : `${uid}.json`;
