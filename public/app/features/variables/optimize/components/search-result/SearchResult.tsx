...
export const SearchResult: React.FC<SearchResultProp> = (props: SearchResultProp) => {
  ...
  let resultContainer;
  if (props.loading) {
    resultContainer = <LoadingPlaceholder text={t('bmc.variables.optimize.loading', 'Loading...')} />;
  } else {
    resultContainer =
      props.resultItems?.length > 0 ? (
        ...
        <EmptySearchResult>
          {props.config?.emptyResultMsg || t('bmc.variables.optimize.no-items', 'No items')}
        </EmptySearchResult>
      );
  }
  ...
  return (
    <>
      <div className={props.containerClassName} style={props.width ? { width: props.width } : undefined}>
        <Stack direction="column">
          ...
          >
            <Icon name={'times'} size={'md'} title={t('bmc.variables.optimize.close-search-panel', 'Close')} />
          </div>
            ...
            <div style={styles.allItemsContainer}>
              <div style={styles.searchInputContainer}>
                <Input
                  ...
                  placeholder={
                    props.config?.searchPlaceholder || t('bmc.variables.optimize.type-to-search', 'Type to search')
                  }
                  value={queryValue}
                ...
                <Button data-testid="domain-picker-search-button" variant={'secondary'} onClick={onSearchClicked}>
                  {props.config?.searchButton || t('bmc.variables.optimize.search', 'Search')}
                </Button>
              </div>
            ...
            <div style={styles.selectedItemsContainer}>
              <div>
                <div style={styles.selectedItemsHeader}>
                  <div>{t('bmc.variables.optimize.selected-domains', 'Selected domains')}</div>
                  <div
                    ...
                    onKeyDown={(ev) => onKeyDown(ev)}
                  >
                    {t('bmc.variables.optimize.deselect-all', 'Deselect all')}
                  </div>
                </div>
