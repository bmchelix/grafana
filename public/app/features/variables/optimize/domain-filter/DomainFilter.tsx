...
export const DomainFilter: React.FC<DomainFilterProps> = (props) => {
  ...
  const onToggleSearch = () => {
    setSearchQuery('');
    // Parent reloads children from API page 0 when opening; reset so pagination UI matches that data after close/reopen.
    setStartPage(1);
    toggleSearch();
      ...
      >
        {props.selected?.length > 0 ? (
          <div style={{ display: 'flex' }}>
            ...
            <div>
              ({props.selected?.length} <Trans i18nKey="bmc.variables.optimize.selected">selected</Trans>)
            </div>
          </div>
        ) : (
          <div>
            <Trans i18nKey="bmc.variables.optimize.no-domain-selected">No Domain Selected</Trans>
          </div>
        )}
        <div
          data-testid="domain-picker-toggle-search"
      ...
      {showSearch && (
        <div
          ...
          style={{
            ...
            zIndex: 10,
            left: '60px',
            top: config.featureToggles.dashboardScene ? theme.spacing(6) : '40px',
