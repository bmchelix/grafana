...
import { detectScript, isExportFooterEnabled, isMultilingualPdfEnabled, Script } from './scriptUtils';
...
function addFooterRow(
  ...
  const firstVisibleField = fields.find((f) => !isFieldHidden(f));

  for (const element of fields) {
    if (isFieldHidden(element)) {
      continue;
    }
...
const getCurrentPanel = (panelId: string): any => {
  if (!configurations.bootData.settings.featureToggles.dashboardScene) {
    ...
    return (window as any).grafanaRuntime.getPanels()?.find((p: any) => {
      if (panelId.includes('panel-')) {
        return p.id === panelId;
      } else {
        return p.id === `panel-${panelId}`;
      }
    });
  }
};
...
export function toCSV(data: DataFrame[], config?: CSVConfig): string {
  ...
  for (let s = 0; s < data.length; s++) {
    ...
    // Grafana 12.x moved footer config from panel.options.footer to fieldConfig
    let footer = false;
    let footerFields: string[] = [];
    let reducerType = '';

    const defaultFooterReducers: string[] = currentPanel?.fieldConfig?.defaults?.custom?.footer?.reducers || [];
    const panelOverrides: any[] = currentPanel?.fieldConfig?.overrides || [];

    if (defaultFooterReducers.length > 0) {
      footer = true;
      reducerType = defaultFooterReducers[0];
      footerFields = fields.filter((f) => !isFieldHidden(f)).map((f) => f.name);
    } else {
      for (const override of panelOverrides) {
        const footerProp = override.properties?.find((p: any) => p.id === 'custom.footer.reducers');
        if (footerProp?.value?.length > 0) {
          footer = true;
          if (!reducerType) {
            reducerType = footerProp.value[0];
          }
          const matcher = override.matcher;
          if (matcher?.id === 'byName' && typeof matcher?.options === 'string') {
            footerFields.push(matcher.options);
          } else if (matcher?.id === 'byNames' && matcher?.options?.names) {
            footerFields.push(...matcher.options.names);
          }
        }
      }
    }

    // Fallback: legacy structure (pre-12.x) for unmigrated panels
    if (!footer && currentPanel?.options?.footer?.show) {
      footer = true;
      footerFields = currentPanel.options.footer.fields || [];
      reducerType = currentPanel.options.footer.reducer?.[0] || '';
    }
    ...
    if (length > 0) {
      ...
      for (let i = 0; i < length; i++) {
        for (let j = 0; j < fields.length; j++) {
          // BMC Code: Start
          // Filtering hidden columns
          if (isFieldHidden(fields[j])) {
            continue;
          }
          // BMC Code: end
          // BMC code: added atleastOneColumnFilled for data rows
...
/**
 * Table panel visibility logic (grafana-ui Table/utils.ts):
 *   - custom.hidden: legacy property (TableRT, pre-12.x overrides)
 *   - custom.hideFrom.viz: current property (table module.tsx registers path "hideFrom.viz";
 *     migrateHiddenFields in table/migrations.ts rewrites custom.hidden → custom.hideFrom.viz)
 */
function isFieldHidden(field: Field): boolean {
  const custom = field.config.custom;
  return Boolean(custom?.hidden || custom?.hideFrom?.viz);
}
