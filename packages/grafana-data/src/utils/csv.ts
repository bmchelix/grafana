...
// BMC Code: Start
export function getFooterValue(field: Field, reducerId: string): any {
...
function addFooterRow(
  ...
  let reducerLabelUsedInField = false;
  const firstVisibleField = fields.find((f) => !f.config.custom?.hidden);
  const firstFieldIsTime = firstVisibleField?.type === FieldType.time;
  ...
  for (const element of fields) {
    ...
    if (isFirstVisible) {
      if (firstFieldIsTime) {
        // First visible column is time → leave it blank
        cellValue = enableOverrides ? getCellStyleWithValue(element, ' ', -1, config, true) : ' ';
      } else if (isFooterField && isNumberField) {
        // First visible column is number & in footerFields → show reducer label with value
        let footerValue = getFooterValue(element, reducerType) ?? ' ';
        ...
        cellValue = enableOverrides
          ...
          : `"${displayValue}"`;
        reducerLabelUsedInField = true; // prevent reuse in later fields
      } else {
        // Not time, not a footer field → show reducer label only
        cellValue = enableOverrides
      ...
      if (isNumberField) {
        let footerValue = getFooterValue(element, reducerType) ?? ' ';
        footerValue = writers[fields.indexOf(element)](footerValue);
        let displayValue = String(footerValue);
        // Only prefix with label if first column is time
        if (firstFieldIsTime && !reducerLabelUsedInField) {
          displayValue = `${footerValue}`;
          reducerLabelUsedInField = true;
        }
...
export function toCSV(data: DataFrame[], config?: CSVConfig): string {
  ...
  for (let s = 0; s < data.length; s++) {
    ...
    const currentPanel = (window as any).grafanaRuntime.getPanels().find((p: any) => p.id === parseInt(panelid, 10));
    ...
    if (config.headerStyle === CSVHeaderStyle.full) {
      ...
      for (let i = 0; i < fields.length; i++) {
        ...
        if (enableOverrides) {
          if (multilingualPdfEnabled && detectedScript === null) {
            ...
            detectedScript = detectScript(fields[i].config.displayName || '');
          }
          const metaStr = getMetaTagForField(fields[i], i, multilingualPdfEnabled);
      ...
      // BMC Code: Start
      const writers = fields.map((f) => makeFieldWriter(f, config));
      if (footer && reducerType && footerFields.length > 0 && enableOverrides && isExportFooter) {
        addFooterRow(fields, footerFields, reducerType, config, footerRow, enableOverrides, writers);
      }
      // BMC Code: end
      csv += config.newline;
    }
...
const getMetaTagForField = (field: Field<any>, index: number, multilingualPdfEnabled: boolean): string => {
