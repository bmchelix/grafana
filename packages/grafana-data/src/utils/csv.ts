...
import { detectScript, isMultilingualPdfEnabled, Script } from './scriptUtils';
  ...
  for (const element of fields) {
    ...
    if (isFirstVisible) {
      if (firstFieldIsTime) {
        ...
        const footerValue = getFooterValue(element, reducerType) ?? ' ';
        const displayValue = `${reducerLabel}: ${footerValue}`;
      ...
      if (isNumberField) {
        ...
        if (firstFieldIsTime && !reducerLabelUsedInField) {
          displayValue = `${reducerLabel}: ${footerValue}`;
  ...
  for (let s = 0; s < data.length; s++) {
    ...
    if (config.headerStyle === CSVHeaderStyle.full) {
      ...
      for (let i = 0; i < fields.length; i++) {
        ...
        if (enableOverrides) {
          if (isMultilingualPdfEnabled() && detectedScript === null) {
            // for detection in column headers
            detectedScript = detectScript(fields[i].config.displayName || '');
          }
          const metaStr = getMetaTagForField(fields[i], i);
      ...
      // BMC Code: Start
      if (footer && reducerType && footerFields.length > 0) {
        addFooterRow(fields, footerFields, reducerType, config, footerRow, enableOverrides);
      }
      // BMC Code: end
      csv += config.newline;
    }
...
const getMetaTagForField = (field: Field<any>, index: number): string => {
  ...
  if (isMultilingualPdfEnabled() && headerInfo.type && index === 0) {
