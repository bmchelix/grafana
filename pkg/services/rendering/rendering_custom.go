...
func (rs *RenderingService) customCSVViaHTTP(ctx context.Context, renderKey string, opts CustomCSVOpts) (*RenderResult, error) {
	...
	opts.Path = rs.getPanelURL(opts.OrgID, opts.UID, opts.PanelId, opts.From, opts.To, opts.Variables, opts.CSVDelimiter, opts.EnableOverrides, opts.HideHeader)
...
func (rs *RenderingService) getPanelURL(orgId int64, uid, panelId, from, to, variables string, csvDelimiter string, enableOverrides string, hideHeader string) string {
	values := url.Values{}
