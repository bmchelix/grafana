...
func (s *Service) GetTeamMembersByIds(c *contextmodel.ReqContext, query *GetTeamMembersByIdsQuery) error {
	return s.store.WithTransactionalDbSession(c.Req.Context(), func(dbSess *sqlstore.DBSession) error {
		...
		if query.Query != "" {
			emailSql, emailArg := s.store.GetDialect().LikeOperator("email", true, query.Query, true)
			nameSql, nameArg := s.store.GetDialect().LikeOperator("name", true, query.Query, true)
			loginSql, loginArg := s.store.GetDialect().LikeOperator("login", true, query.Query, true)
			sess.Where(fmt.Sprintf("(%s OR %s OR %s)", emailSql, nameSql, loginSql), emailArg, nameArg, loginArg)
		}
		...
		sess.Select(fmt.Sprintf(`team_member.org_id,
    		...
    		user_auth.auth_module`, s.store.GetDialect().Quote("user")))
