...
func ProvideOSSService(
	...
	db db.DB, permRegistry permreg.PermissionRegistry, teamService team.Service, lock *serverlock.ServerLockService,
) *Service {
	s := &Service{
...
func (s *Service) HasRequiredPermissions(ctx context.Context, orgID, userID int64, requiredRole string, requiredPermissions []string) (bool, error) {
	...
	if requiredPermissions != nil && len(requiredPermissions) > 0 {
		if s.teamService != nil {
			teamIDs, terr := s.teamService.GetTeamIDsByUser(ctx, &team.GetTeamIDsByUserQuery{OrgID: orgID, UserID: userID})
			if terr != nil {
				return false, terr
			}
			user.Teams = teamIDs
