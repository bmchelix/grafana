...
func (s *ServiceImpl) addAppLinks(treeRoot *navtree.NavTreeRoot, c *contextmodel.ReqContext) error {
	...
	isPluginEnabled := func(plugin pluginstore.Plugin) bool {
		...
		// BMC Code: to use tenant-level feature-flag to determine application availbiity
		if plugin.ID == "bmc-insightfinder-app" {
			isInsightFinderEnabled := external.FeatureFlagInsightFinder.Enabled(c.Req, c.SignedInUser)
			return isInsightFinderEnabled
		}
		// BMC Code: End
