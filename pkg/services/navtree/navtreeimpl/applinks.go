...
func (s *ServiceImpl) addAppLinks(treeRoot *navtree.NavTreeRoot, c *contextmodel.ReqContext) error {
	...
	for _, plugin := range s.pluginStore.Plugins(c.Req.Context(), plugins.TypeApp) {
		...
		if plugin.ID == "bmc-insightfinder-app" {
			...
			if !s.sqlStore.IsFeatureEnabled(c.Req.Context(), c.OrgID, "Enable Insight Finder") {
				continue // BMC Code: If user does not have insight finder permission, do not show the plugin
			}
