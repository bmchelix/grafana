...
// checkIfUserFromExternalOrg: This function will check if the user is from external org
// by checking the presence of org claim in JWT token. If org claim is present, then it
// will set HasExternalOrg flag in identity context to true and also set the list of orgs
// in MspOrgs field in identity context. If org claim is not present, then it will set
// HasExternalOrg flag to false and MspOrgs list will be empty.
...
// CreateUnrestrictedTeam: This function will create a team with name "Unrestricted Access"
// for the users who have access to all orgs in MSP tenant. This will help us to assign
// permissions at this team level for unrestricted users in MSP tenant.
...
// UpdateTeamMembership: This function will update team membership for the user
// based on the groups claim in JWT and also based on the MSP orgs. For MSP orgs,
// we will be adding users to teams with teamID format as "orgID_orgString"
// (e.g. 1_1001, 1_1002 etc) and for groups claim in JWT, we will be adding users
// to teams with teamID as group claim value. For unrestricted users in MSP tenant,
// we will be adding them to "Unrestricted Access" team which has teamID in format
// "orgID_00" (e.g. 1_00).
