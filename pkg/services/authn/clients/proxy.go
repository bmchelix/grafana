...
func ProvideProxy(cfg *setting.Cfg, cache proxyCache, tracer trace.Tracer, orgService org.Service, clients ...authn.ProxyClient) (*Proxy, error) {
	list, err := parseAcceptList(cfg.AuthProxy.Whitelist)
	if err != nil {
		return nil, err
	}
	// BMC Change: Inline injected orgservice
	return &Proxy{log.New(authn.ClientProxy), cfg, cache, clients, list, tracer, orgService}, nil
}
...
type Proxy struct {
	...
	tracer      trace.Tracer
	// BMC Change: next line injected orgservice
...
// BMC Change: Next line inline: Add new argument inDedicatedInst
