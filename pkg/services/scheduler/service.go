...
var failureSchedulerBhdCode = "bhd-01574"
...
func getErrorFromBody(body []byte, status string) error {
	var schedulerResp schedulerResponse
	if err := json.Unmarshal(body, &schedulerResp); err == nil {
		// Handled for only one specific usecase for Repeated Panel Limit Exceeded rest all will result as Failed to generate report
		// Need to handle other usecases in future and remove schedulerResp.BHDCode != failureSchedulerBhdCode
		if schedulerResp.BHDCode != "" && schedulerResp.BHDCode != failureSchedulerBhdCode {
			return errors.New(schedulerResp.ErrorMessage)
		}
	}
	return ErrSchedulerServiceFailedToPreviewPDF
}
...
func PreviewPDF(data dtos.RSDataPreview) ([]byte, int, error) {
	...
	if res.StatusCode != 200 {
		...
		err := getErrorFromBody(body, res.Status)
