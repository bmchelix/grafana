...
func (ss *sqlStore) GetRolesAndPermissionsService(ctx context.Context, orgID int64, dashboardPermissionsService accesscontrol.DashboardPermissionsService, folderPermissionService accesscontrol.FolderPermissionsService, user_id string) (usagedata.RolesPermissionsResponse, error) {
	...
	if user_id == "" {
		ss.log.Error(usagedata.ErrUserIdNotFound.Error())
...
GROUP BY user_id, 
    ...
    updated_by;
		...
		if result.Len() == 0 {
			ss.log.Error(usagedata.ErrNoRolesAndPermissionsFound.Error())
			return usagedata.ErrNoRolesAndPermissionsFound
		}
		return nil
	})
	if err != usagedata.ErrNoRolesAndPermissionsFound || err != nil {
		return result, err
	}
	// if err != nil {
	// 	ss.log.Error("Failed to get roles and permissions", "error", err)
	// 	return result, nil
	// }
...
func (ss *sqlStore) GetDashboardsRepoSchedule(ctx context.Context, fromTime string, toTime string, orgID int64, lastDayScheduleDetails bool, allScheduleInfo bool) (usagedata.UsageDataResponse, error) {
	...
	err := ss.db.WithDbSession(ctx, func(dbSess *db.Session) error {
		...
		ss.log.Info(fmt.Sprintf("Running SQL query to fetch reports schedule info with query params Detailed %v and totalExecInfo %v for Org ID %v", lastDayScheduleDetails, allScheduleInfo, orgID))
		...
		if allScheduleInfo {
			...
			if err != nil {
				ss.log.Error(fmt.Sprintf("Error while running SQL query to Schedule Statistics for OrgID %d", orgID))
				return err
			}
			if allScheduleInfoResponse.Len() == 0 {
...
func (ss *sqlStore) GetOrgLevelDashboardStatistics(ctx context.Context, fromTime string, toTime string, orgID int64) (usagedata.OrgLevelDashboardStatisticsResponse, error) {
	...
	err := ss.db.WithDbSession(ctx, func(dbSess *db.Session) error {
		...
		ss.log.Info("Running SQL query to fetch org level dashboard statistics for OrgID ", orgID, " , from time = ", fromTime, " , to time = ", toTime)
...
func (ss *sqlStore) GetActiveDashboardsCount(ctx context.Context, orgID int64) (usagedata.ActiveDashboardsCountResponse, error) {
	...
	err := ss.db.WithDbSession(ctx, func(sess *db.Session) error {
		ss.log.Info("Running SQL query to fetch active dashbaords count")
		rawSQL := `select count (dashboard_id) as act_dash_count from metric_schema.grafana_bmc_hdb_api_dashboard_hit_labels
		           where tenant_id = ?`
		err := sess.SQL(rawSQL, orgID).Find(&response.Data)
		if err != nil {
			ss.log.Error("Error while running SQL query to active dashbaords count")
			return err
		}
		return nil
	})
	...
	ss.log.Info("Ran SQL query to fetch active dashbaords. count = ", response.Data)
	return response, nil
}
...
func (ss *sqlStore) GetSchedulerStaging(ctx context.Context, orgID int64, scheduleName string, fromTime string, toTime string, isDev bool) (usagedata.ScheduleStagingResponse, error) {
	...
	err := ss.db.WithDbSession(ctx, func(dbSess *db.Session) error {
		ss.log.Info(fmt.Sprintf("Running SQL query to fetch report scheduler staging info and get errors %v for org - %v", isDev, orgID))
		...
		if err != nil {
			ss.log.Error(fmt.Sprintf("Error while Running SQL query to fetch report scheduler staging info for org - %v", orgID))
			return err
		}

		if result.Len() == 0 {
			ss.log.Info(fmt.Sprintf("No scheduled staging found for org - %v", orgID))
...
func (ss *sqlStore) GetNextSchedules(ctx context.Context, orgID int64) (usagedata.ScheduleShortInfoResponse, error) {
	...
	err := ss.db.WithDbSession(ctx, func(dbSess *db.Session) error {
		...
		ss.log.Info(fmt.Sprintln("Running SQL query to fetch reports info for last 24 hours"))
		...
		ss.log.Info("Fetched scheduler info for next 24. Returning data")
		return nil
	})
	if err != nil {
		return result, err
	}
	ss.log.Info("Ran SQL query to fetch scheduler info for next 24 hours. Returning")
	return result, nil
}
