...
type store interface {
	...
	GetDashboardHitsUserInfo(ctx context.Context, fromTime string, toTime string, orgID int64, user string, dashboard string) (usagedata.DashboardHitCountWithUserInfoResponse, error)
...
func (ss *sqlStore) GetDashboardHitsUserInfo(ctx context.Context, fromTime string, toTime string, orgID int64, user string, dashboard string) (usagedata.DashboardHitCountWithUserInfoResponse, error) {
	var result usagedata.DashboardHitCountWithUserInfoResponse
	err := ss.db.WithDbSession(ctx, func(dbSess *db.Session) error {

		ss.log.Info(fmt.Sprintf("Running SQL query to fetch Dashboard Hit User Info for OrgID %d", orgID))
		rawSQL := `
		SELECT
			%s,
			a.data_delta,
		...
		rawSQL = fmt.Sprintf(rawSQL, idQuery, nameQuery, condition)
		err := dbSess.SQL(rawSQL, orgID, fromTime, toTime).Find(&result.Data)

		if err != nil {
			ss.log.Error(fmt.Sprintf("Error while running SQL query to fetch Dashboard Hit User Info for OrgID %d", orgID))
			return err
		}
		if result.Len() == 0 {
			ss.log.Warn(fmt.Sprintf("No Data fetched for Dashboard Hit User Info for OrgID %d", orgID))
			result = usagedata.DashboardHitCountWithUserInfoResponse{
				Data: make([]usagedata.DashboardHitCountWithUserInfo, 0),
			}
		}
		return nil
	})
