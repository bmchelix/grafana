...
func addUserMigrations(mg *Migrator) {
	...
	// BMC code: update uid length to 20
	mg.AddMigration("Make sure users uid are set", NewRawSQLMigration("").
		...
		Postgres("UPDATE `user` SET uid='u' || lpad('' || id::text,20,'0') WHERE uid is NULL OR uid = '';").
		Mysql("UPDATE user SET uid=concat('u',lpad(id,20,'0')) WHERE uid is NULL OR uid = '';"))
