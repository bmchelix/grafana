...
func addFeatureStatusMigrations(mg *Migrator) {
	...
	// Enable Insight Finder
	mg.AddMigration("Add feature flag for enabling Insight Finder application",
		NewRawSQLMigration(`
			INSERT INTO feature_status (org_id, feature_name, status)
			VALUES(1, 'Enable Insight Finder', 'true')`),
	)
	...
	// Enable RTL Support
	mg.AddMigration("Add feature flag for enabling Right-to-Left Interface",
		NewRawSQLMigration(`
			INSERT INTO feature_status (org_id, feature_name, status)
			VALUES(1, 'Right-to-Left Interface', 'true')`),
	)
}
