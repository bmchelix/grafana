...
func AddRolePermissionRbacTable(mg *mig.Migrator) {
	...
	var newInsightFinderDeepAnalysisPermissions = []BhdRolePermission{
		{"Insight Finder", "insightfinder.deepanalysis:access", "Deep Analysis", "AI-driven root cause and outlier detection engine. Deep analysis needs to be configured with HelixGPT Agent Studio version 26.3.00 or above", true},
	}
	rawSQL = addDefaultPermissionForBuiltInRole(1, newInsightFinderDeepAnalysisPermissions)
	mg.AddMigration("bhd: insert insight finder deep analysis as default permissions for builtin role Admin", mig.NewRawSQLMigration(rawSQL))
