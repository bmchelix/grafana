...
func AddPermissionRbacTable(mg *mig.Migrator) {
	...
	mg.AddMigration("bhd: insert new permission for insight finder deep analysis", mig.NewRawSQLMigration(insertQueryForInsightFinderDeepAnalysisPermission()))
}
...
func insertQueryForInsightFinderDeepAnalysisPermission() string {
	var newPermissions = []BhdRolePermission{
		{"Insight Finder", "insightfinder.deepanalysis:access", "Deep Analysis", "AI-driven root cause and outlier detection engine. Deep analysis needs to be configured with HelixGPT Agent Studio version 26.3.00 or above", false},
	}
