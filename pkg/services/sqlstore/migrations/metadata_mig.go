...
// updateViewDescription generates a SQL UPDATE statement for updating a metadata view's description.
// It sanitizes the description input, updates the timestamp to current UTC time, and increments the version.
// Returns a formatted SQL string for the rms_metadata_view_list table where tenant_id = 1.
func updateViewDescription(name string, description string) string {
	const dbDateTimeFormat = "2006-01-02 15:04:05 -0700"
	cleanDescriptString := cleanString(description)[0].(string) // Extract the first element from the cleaned string slice
	updateTime := time.Now().Format(dbDateTimeFormat)
	queryTemplate := `UPDATE rms_metadata_view_list
	SET description = '%s', updated = '%s', version = version + 1
	WHERE name = '%s' AND tenant_id = 1`
	return fmt.Sprintf(queryTemplate, cleanDescriptString, updateTime, name)
}
...
func addMetaDataView(mg *Migrator) {
	...
	// Version column to track the version of the view
	mg.AddMigration("Add column version to the table", NewAddColumnMigration(metaDataTableV1, &Column{
		Name: "version", Type: DB_BigInt, Nullable: false, Default: "1",
	}))

	// Columns for Custom view Agent
	...
	mg.AddMigration("Add column base_view_version to the table", NewAddColumnMigration(metaDataTableV1, &Column{
		Name: "base_view_version", Type: DB_BigInt, Nullable: true,
	}))
	...
	mg.AddMigration("Customer Service Management : Customer Service Management V1",
		NewRawSQLMigration(fmt.Sprintf(`
		  INSERT INTO rms_metadata_view_list (name, tenant_id, user_id, file_key, itsm_comp_version, created, updated)
		  VALUES('Customer Service Management', 1, 1, 'Customer_Service_Management','22_1_04', '%s', '%s')`, formattedTs, formattedTs)),
	)
	...
	mg.AddMigration("UPDATE rms_metadata_view_list.Customer Service Management: set description V1",
		NewRawSQLMigration(fmt.Sprintf(`UPDATE rms_metadata_view_list
			SET description = '%s'
			WHERE name = 'Customer Service Management' AND tenant_id = 1`, cleanString(CustomerServiceManagementDescription)...),
		),
	)
	...
	mg.AddMigration("UPDATE rms_metadata_view_list.Asset Management: set description V4",
		NewRawSQLMigration(fmt.Sprintf(`UPDATE rms_metadata_view_list
			SET description = '%s'
			WHERE name = 'Asset Management' AND tenant_id = 1`, cleanString(AssetManagementDescription)...),
		),
	)
	mg.AddMigration("UPDATE rms_metadata_view_list.Configuration Management GenAI Ready: set description V4",
		NewRawSQLMigration(fmt.Sprintf(`UPDATE rms_metadata_view_list
			SET description = '%s'
			WHERE name = 'Configuration Management GenAI Ready' AND tenant_id = 1`, cleanString(ConfigurationManagementDescription)...),
		),
	)
	mg.AddMigration("UPDATE rms_metadata_view_list.Work Order Management view: set description V1",
		NewRawSQLMigration(fmt.Sprintf(`UPDATE rms_metadata_view_list
			SET description = '%s'
			WHERE name = 'Work Order Management' AND tenant_id = 1`, cleanString(WorkOrderManagementDescription)...),
		),
	)
	mg.AddMigration("UPDATE rms_metadata_view_list.Asset Management: set description V5",
		NewRawSQLMigration(fmt.Sprintf(`UPDATE rms_metadata_view_list
			SET description = '%s'
			WHERE name = 'Asset Management' AND tenant_id = 1`, cleanString(AssetManagementDescription)...),
		),
	)
	// Use updateViewDescription function to update the description of the view
	mg.AddMigration("UPDATE rms_metadata_view_list.Asset Management: set description V6",
		NewRawSQLMigration(updateViewDescription("Asset Management", AssetManagementDescription)),
	)

	mg.AddMigration("UPDATE rms_metadata_view_list.Configuration Management: set description V1",
		NewRawSQLMigration(updateViewDescription("Configuration Management", ConfigurationManagementDescription)),
	)
	mg.AddMigration("DELETE rms_metadata_view_list.Configuration Management GenAI Ready: set is_deleted to true",
		NewRawSQLMigration(`UPDATE rms_metadata_view_list
			SET is_deleted = true 
			WHERE name = 'Configuration Management GenAI Ready' 
			AND tenant_id = 1
	   `),
	)
