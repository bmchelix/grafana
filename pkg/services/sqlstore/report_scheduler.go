...
func (ss *SQLStore) DeleteUserFromRS(ctx context.Context, query *models.DeleteUserFromRS) error {
	return ss.WithTransactionalDbSession(ctx, func(sess *DBSession) error {
		...
		WHERE ((recipients IS NULL OR recipients = '') AND (bcc_recipients IS NULL OR bcc_recipients = '') AND schedule_type = 'email')`)
...
func updateRSData(sess *DBSession, query *models.UpdateRS) error {
	...
	if _, err := sess.Table("report_data").
		...
		Nullable(nullable...).
		UseBool("has_date_stamp", "has_time_stamp", "no_data_condition", "compress_attachment", "table_scaling").
