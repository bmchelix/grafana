...
const systemAuditUser = "system"

// SystemReqContext returns a request context for background operations (e.g. cleanup purge)
// where audit events should be attributed to the system user.
func SystemReqContext(orgID int64) *contextmodel.ReqContext {
	return &contextmodel.ReqContext{
		Context: &web.Context{Req: &http.Request{Method: http.MethodDelete}},
		SignedInUser: &user.SignedInUser{
			OrgID: orgID,
			Login: systemAuditUser,
			Name:  systemAuditUser,
		},
	}
}
...
func DashboardDeleteAudit(c *contextmodel.ReqContext, err error, dash ...*dashboards.Dashboard) {
	if err == nil {
		for _, dashboard := range dash {
			sendAudit(kp.EventOpt{Ctx: c, ObjectName: dashboard.Title, ObjectDetails: "Dashboard permanently deleted.", OperationSubType: "Dashboard " + dashboard.Title + " deleted successfully."}, kp.DashboardDeleteAudit)
		}
	} else {
		for _, dashboard := range dash {
...
func DashboardSoftDeleteAudit(c *contextmodel.ReqContext, err error, dash ...*dashboards.Dashboard) {
	if err == nil {
		for _, dashboard := range dash {
			sendAudit(kp.EventOpt{Ctx: c, ObjectName: dashboard.Title, ObjectDetails: "Dashboard moved to archive.", OperationSubType: "Dashboard " + dashboard.Title + " moved to archive."}, kp.DashboardSoftDeleteAudit)
		}
	} else {
		for _, dashboard := range dash {
