...
var (
	...
	// author (kmejdi) end
	EnvType string
	// When true, use HCG URL dropdown for external datasource configuration.
	IsExternalDSUrlDropdownEnabled bool
	// End
	// BMC code
	// feature flag
...
func (cfg *Cfg) parseINIFile(iniFile *ini.File) error {
	...
	// BMC: toggles HCG external datasource URL dropdown (Helm: GF_IS_EXTERNAL_DS_URL_DROPDOWN_ENABLED).
	IsExternalDSUrlDropdownEnabled = true
	if v := strings.TrimSpace(os.Getenv("GF_IS_EXTERNAL_DS_URL_DROPDOWN_ENABLED")); v != "" {
		IsExternalDSUrlDropdownEnabled, _ = strconv.ParseBool(v)
	}
	// End
