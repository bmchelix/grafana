...
func GetAllMatchingKeysFromCluster(ctx context.Context, pattern string) ([]string, error) {
	...
	result, err := redisCircuitBreaker.Execute(func() (interface{}, error) {
		...
		redisVarCacheLogger.Debug("getting master for key", "pattern", pattern)
		masterClient, err := clusterClient.MasterForKey(timeoutCtx, pattern)
		if err != nil {
			redisVarCacheLogger.Error("failed to get master for key", "pattern", pattern, "error", err)
			return nil, err
		}
		redisVarCacheLogger.Debug("master client found successfully", "masterClient", masterClient)
		var allKeys []string
		var cursor uint64

		redisVarCacheLogger.Debug("scanning master client", "pattern", pattern)
		for {
			var keys []string
			var err error
		...
		redisVarCacheLogger.Info("scanned single shard", "pattern", pattern, "totalKeys", len(allKeys))
		return allKeys, nil
	})
