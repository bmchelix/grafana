...
func isPlatformQuery(proxy *DataSourceProxy) bool {
	return strings.Contains(proxy.proxyPath, "api/arsys") || strings.Contains(proxy.proxyPath, "rx/application/chat/helixgpt") || strings.Contains(proxy.proxyPath, "api/rx/application/systemconfiguration")
}
...
func (proxy *DataSourceProxy) HandleRequest() {
	...
	if enableCachingFeatureFlag {
		...
		if remoteVariableCacheSettings.Enabled && enableCachingValueFromReq && dashboardUID != "" && variableName != "" {
			...
			queryCacheKey = GenerateQueryKey(proxy.ctx.OrgID, dashboardUID, variableName, proxy.ctx.UserID)
			if queryCacheKey != "" {
