...
func (proxy *DataSourceProxy) HandleRequest() {
	...
	// ( Disable SmartGraph API)
	disableSmartGraph := proxy.isSmartGraphDisabled(proxy.ctx.Req, proxy.ctx.SignedInUser)

	if disableSmartGraph && strings.Contains(proxy.proxyPath, "smart-graph-api/api") {
		proxy.ctx.JsonApiErr(403, "SmartGraph API is disabled", nil)
		return
	}
...
func (proxy *DataSourceProxy) director(req *http.Request) {
	...
	if proxy.proxyPath == "api/usagedata" && proxy.ds.Type == datasources.DS_BMC_JSON {
		...
		if username != "" && password != "" {
			...
			req.URL.Scheme = "http" // This can be ignored for FIPS since it is local adereporting call
			req.URL.Host = proxy.cfg.HTTPAddr + ":" + proxy.cfg.HTTPPort
			req.Header.Set("X-DS-Authorization", "")
			req.Header.Set("Authorization", "Basic "+encodedAuth)
			req.Header.Set("X-Jwt-Token", "")
		}
	}
	// Usage data code ends
	// Still BMC code
	...
	if len(dsAuth) > 0 {
		...
		// BMC code starts
		// DS Request Elevation code starts
		elevationJwtToken := proxy.getJWTTokenForElevatedRequest()
		if elevationJwtToken != "" {
			HelixJwtToken = elevationJwtToken
		}
		// DS Request Elevation code ends
...
func (proxy *DataSourceProxy) isSmartGraphDisabled(req *http.Request, signedInUser *user.SignedInUser) bool {
	return external.FeatureFlagDisableSmartGraph.Enabled(req, signedInUser)
}
