...
// authenticateUser validates the authentication and returns only error if authentication is invalid
func authenticateUser(c *contextmodel.ReqContext) (*authz.UserInfo, string, error) {
	imsJWTToken := c.Req.Header.Get("X-Jwt-Token")
	imsJWTTokenHeader := c.Req.Header.Get("IMS-JWT-Token")
	if imsJWTTokenHeader != "" {
		imsJWTToken = imsJWTTokenHeader
	...
	if imsJWTToken == "" {
		return nil, "", fmt.Errorf("missing IMS Token")
	}
	decodedToken, err := authz.Authorize(imsJWTToken)
	if err != nil || decodedToken == nil {
		return nil, "", fmt.Errorf("invalid IMS Token")
	}
	return decodedToken, imsJWTToken, nil
}

// CustomRenderAgentPanel handles POST /render/agent/panel - forwards request to renderer service
func (hs *HTTPServer) CustomRenderAgentPanel(c *contextmodel.ReqContext) {
	// since this method is called by passing authentication, create a new helper function to validate the authentication which returns only error if authentication is invalid
	decodedToken, imsJWTToken, err := authenticateUser(c)
	if err != nil {
		c.Handle(hs.Cfg, 401, "Invalid Authentication", err)
		return
	}
	orgIdHeader := c.Req.Header.Get("X-Grafana-Org-Id")
	orgId, _ := strconv.ParseInt(orgIdHeader, 10, 64)
	tenantId, _ := strconv.ParseInt(decodedToken.Tenant_Id, 10, 64)
	if tenantId != orgId {
		c.Handle(hs.Cfg, 401, "Invalid Org ID", nil)
		return
	}
	userId, _ := strconv.ParseInt(decodedToken.UserID, 10, 64)
	...
	if err != nil {
		c.Handle(hs.Cfg, 400, "Invalid request body", err)
		return
	}
	...
	var req struct {
		...
		MappedAgent string `json:"mapped_agent"`
	}

	if err := json.Unmarshal(body, &req); err != nil {
		c.Handle(hs.Cfg, 400, "Invalid req parameters", err)
		return
	}

	if req.Query == "" || req.Datasource == "" || req.PanelType == "" {
		c.Handle(hs.Cfg, 400, "Missing required fields: query, datasource, panelType", nil)
		return
	}
	...
	timeoutStr := c.Req.URL.Query().Get("timeout")
	if timeoutStr == "" {
		timeoutStr = "600"
	}
	...
	width, err := strconv.Atoi(queryReader.Get("width", "1200"))
	if err != nil {
		c.Handle(hs.Cfg, 400, "Render parameters error", fmt.Errorf("cannot parse width as int: %s", err))
		return
	}
	...
	scale, err := strconv.ParseFloat(queryReader.Get("scale", "0.9"), 64)
	if err != nil {
		c.Handle(hs.Cfg, 400, "Render parameters error", fmt.Errorf("cannot parse scale as float: %s", err))
		return
	}
	...
	headers.Set("IMS-JWT-Token", "Bearer "+imsJWTToken)
	headers.Set("Content-Type", "application/json")

	result, err := hs.RenderService.CustomRenderAgentPanel(c.Req.Context(), rendering.CustomAgentPanelOpts{
		TimeoutOpts: rendering.TimeoutOpts{
			Timeout: time.Duration(timeout) * time.Second,
		},
		AuthOpts: rendering.AuthOpts{
		...
		PanelTitle:        req.PanelTitle,
		MappedAgent:       req.MappedAgent,
		Width:             width,
		Height:            height,
		DeviceScaleFactor: scale,
	}, nil)
