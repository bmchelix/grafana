...
func ValidateURL(typeName, urlStr string) (*url.URL, error) {
	...
	default:
		...
		if !reURL.MatchString(urlStr) {
			logger.Debug(
				"Data source URL doesn't specify protocol, so prepending it with https:// in order to make it unambiguous",
				"type", typeName, "url", urlStr) // BMC change - make https the default protocol - FIPS mode
			urlStr = fmt.Sprintf("https://%s", urlStr) // BMC change - make https the default protocol - FIPS mode
		}
...
func TCPPing(rawURL string) error {
	// If there's no scheme, url.Parse will misinterpret host:port.
	// Add a scheme so the parser works correctly.
	...
	if err != nil || u.Host == "" {
		return fmt.Errorf("invalid URL or host: %q", rawURL)
	}
...
// Start
func ValidateSSRF(cmdType string, urlStr string, platformURL string, orgId int64, req *http.Request, signedInUser *user.SignedInUser) (*url.URL, error) {
	...
	if dsToValidateStr == "" {
		...
		dataSources := strings.SplitSeq(dsToValidateStr, ",")
	...
	if shouldValidate {
		...
		if externalds.IsExternalDs(cmdType) && external.FeatureFlagExternalDatasource.Enabled(req, signedInUser) {
			err = TCPPing(urlStr)
		} else {
			err = Ping(urlStr)
		}
