...
func (hs *HTTPServer) SetFTPConfig(c *contextmodel.ReqContext) response.Response {
	...
	// BMC FIPS changes - encrypt before storing
	encryptedPwd, err := ftp.EncryptPassword(c.Req.Context(), cmd.Password, hs.SecretsService)
	if err != nil {
		return response.Error(http.StatusInternalServerError, "Failed to encrypt password", err)
	}
	cmd.Password = encryptedPwd
...
func (hs *HTTPServer) SetDefaultFTPConfig(c *contextmodel.ReqContext) response.Response {
	...
	if err := hs.sqlStore.GetFTPConfig(c.Req.Context(), preValue, false); err != nil {
...
func (hs *HTTPServer) ModifyFTPConfig(c *contextmodel.ReqContext) response.Response {
	...
	// BMC FIPS changes - encrypt before storing
	encPwd, err := ftp.EncryptPassword(c.Req.Context(), cmd.Password, hs.SecretsService)
	if err != nil {
		return response.Error(http.StatusInternalServerError, "Failed to encrypt FTP password", err)
	}
	cmd.Password = encPwd
	...
	if err := hs.sqlStore.GetFTPConfig(c.Req.Context(), newValue, false); err != nil {
...
func (hs *HTTPServer) DecryptPassword(c *contextmodel.ReqContext) response.Response {
	cmd := &models.FtpDecryptedPasswordRequest{}
	if err := web.Bind(c.Req, cmd); err != nil {
		return response.Error(http.StatusBadRequest, "bad request payload while decrypting password", err)
	}

	// using ftp config id and org id to get the password
	ftpConfig := &models.GetFTPConfig{
		Id:    cmd.ID,
		OrgId: c.OrgID,
	}
	if err := hs.sqlStore.GetFTPConfig(c.Req.Context(), ftpConfig, true); err != nil {
		return response.Error(http.StatusInternalServerError, "failed to get FTP configuration with id: "+strconv.FormatInt(cmd.ID, 10), err)
	}

	// if ftp config id not found
	if ftpConfig.Result.Id == 0 {
		return response.Error(http.StatusInternalServerError, "FTP configuration not found", nil)
	}

	// if ftp config id found but password is empty
	if !ftpConfig.Result.HasPassword {
		return response.Error(http.StatusInternalServerError, "FTP configuration does not have a password", nil)
	}

	// decrypt the password
	decryptedPwd, err := ftp.DecryptPassword(c.Req.Context(), ftpConfig.Result.Password, hs.SecretsService)
	if err != nil {
		return response.Error(http.StatusInternalServerError, "failed to decrypt password: "+err.Error(), err)
	}
	return response.JSON(
		http.StatusOK,
		&models.FtpDecryptedPasswordResponse{
