...
// For insightfinder we do not want some views like CSM to get picked by default since it conflicts with other OOTB views
// If customer wants to use any of these, he must select this in the Configure Views page and save them as their preference
// Only OOTB views can be added here, we need to mention view file key from DB
// In future if we blacklist more than one, change this to array
const CSMFileKey = "Customer_Service_Management"
...
func (hs *HTTPServer) GetViewList(c *contextmodel.ReqContext) response.Response {
	...
	for _, view := range views {
		...
		if insightFinderEnabled {
			// If request is from Insight Finder it comes in this block
			if enabledViewIDs == nil {
				// If request is from IF + Customer has not set any prferences, it comes in this block
				// if tenant has not set any preferences, return only ootb, undeleted and with description
				// Blacklisting CSM view due to context confusions with other OOTB views
				if view.TenantID != 1 || view.Description == "" || view.FileKey == CSMFileKey {
					continue
				}

			} else {
				// If request is from IF and customer has set preferences, it comes in this block
		...
		if extend {
			...
			if view.BaseViewVersion.Valid {
				v.BaseViewVersion = view.BaseViewVersion.Int64
			}
			v.Created = view.Created
			v.Updated = view.Updated
		}
...
func (hs *HTTPServer) GetViewJSON(c *contextmodel.ReqContext) response.Response {
	fileKey := c.Query("fileKey")
	if fileKey == "" {
		hs.log.Error("File key is required")
		return hs.FailResponse(models.ErrViewNotFound)
	}
	...
	imsToken, err := GetIMSToken(c, c.OrgID, c.UserID)
	if err != nil {
		hs.log.Error("Failed to get JWT token", err)
		return hs.FailResponse(err)
	}
	...
	path := rmsURL + "/" + "reportingmetadata/api/v1/load_view"

	resp, err := hs.rmsMetadataService.Get(path, headers, queryParams)
	if err != nil {
		hs.log.Error("Failed to get view json from RMS", err)
		return hs.FailResponse(err)
	}
	return hs.SuccessResponse(resp)
}

func (hs *HTTPServer) DeleteView(c *contextmodel.ReqContext) response.Response {
	viewId, err := strconv.ParseInt(web.Params(c.Req)[":viewId"], 10, 64)
	if err != nil {
		hs.log.Error("Failed to parse view id", err)
		return hs.FailResponse(models.ErrViewNotFound)
	}
	...
	path := rmsURL + "/" + "reportingmetadata/api/v1/delete_view"
	queryParams := map[string]int64{
		"viewId": viewId,
	}

	errorResponse := hs.rmsMetadataService.Delete(path, headers, queryParams)
	if errorResponse != nil {
		hs.log.Error("Failed to delete view from RMS", errorResponse)
		return hs.FailResponse(errorResponse)
	}
