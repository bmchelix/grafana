...
func GetTenantReBranding(c *contextmodel.ReqContext) response.Response {
	...
	if referer != "" {
		...
		if u, err := url.Parse(referer); err == nil {
			...
			rebrandingLogger.Info("Request triggered", "referer", cleanReferer)
			// Assign only host part to outgoing request
			host = u.Host
		} else {
			rebrandingLogger.Warn("Failed to parse referer", "raw", referer)
		}
	}
	mainCss := getMainCss(host)
...
func getMainCss(host string) string {
	epoch := time.Now().Unix()
