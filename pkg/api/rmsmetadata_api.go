...
func (hs *HTTPServer) registerRMSMetadataRoutes() {
	...
	r.Group("/api/rmsmetadata", func(apiRoute routing.RouteRegister) {
		...
		apiRoute.Group("/view", func(userPersonaRoute routing.RouteRegister) {
			...
			userPersonaRoute.Get("/get_view_json", rbac.CanAccessViews, routing.Wrap(hs.GetViewJSON))
			...
			userPersonaRoute.Delete("/delete_view/:viewId", rbac.CanAccessViews, routing.Wrap(hs.DeleteView))
		})
