...
type Tenantfeatures struct {
	...
	HCGNamespace string `json:"HCGNamespace,omitempty"`
}
...
// GetHCGNamespace retrieves the HCG namespace for the given tenant from the feature flag service.
// It fetches tenant features (with caching) and returns the HCGNamespace field
func GetHCGNamespace(req *http.Request, signedInUser *user.SignedInUser) string {
	if !setting.FeatureFlagEnabled {
		return ""
	}

	cacheInstance := authz.GetInstance()
	cacheKey := "hcg_namespace_" + strconv.Itoa(int(signedInUser.OrgID))
	...
	namespace := ""
	for _, tf := range tenantFeatures {
		if tf.Name == FeatureFlagExternalDatasource.String() && tf.Status && tf.HCGNamespace != "" {
			namespace = tf.HCGNamespace
			break
		}
	}

	cacheInstance.Set(cacheKey, namespace, 60*time.Minute)
	return namespace
}
