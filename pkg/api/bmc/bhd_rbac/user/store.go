...
func SearchUsers(ctx context.Context, store sqlstore.SQLStore, query *SearchUsersQuery) (*SearchUserQueryResult, error) {
	...
	err := store.WithDbSession(ctx, func(sess *db.Session) error {
		...
		if query.Query != "" {
			emailSql, emailArg := store.GetDialect().LikeOperator("email", true, query.Query, true)
			nameSql, nameArg := store.GetDialect().LikeOperator("name", true, query.Query, true)
			loginSql, loginArg := store.GetDialect().LikeOperator("login", true, query.Query, true)
