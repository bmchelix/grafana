...
// ExtractLocalesFromJSON extracts locales from dashboard JSON (e.g. from K8s dashboard spec).
// Used when HTTP request is not available (e.g. in storage hooks).
func ExtractLocalesFromJSON(dash *simplejson.Json) LocalesJSON {
	localesJson := LocalesJSON{Locales: make(map[Locale]ResourceLocales)}
	if dash == nil {
		return localesJson
	}
	...
	for key, value := range localeMap {
		...
		if data, ok := value.(map[string]interface{}); ok {
			if nameValue, ok := data["name"].(string); ok {
				localesJson.Locales[localeKey] = ResourceLocales{
					Name: nameValue,
				}
			}
		}
