...
func (p *PluginsAPI) ExportPlugin(c *contextmodel.ReqContext) response.Response {
	...
	displayName := fmt.Sprintf("%s-%d.json", dashboard.Title, time.Now().UnixMilli())

	headers := http.Header{}
	headers.Set("Content-Type", "application/json")
	headers.Set("Content-Disposition", "attachment; filename=\""+strings.ReplaceAll(displayName, `"`, "_")+"\"")
	// X-Export-Filename carries the full Unicode title as a URL-encoded string so the
	// browser JS can decode it correctly regardless of HTTP header encoding limitations.
	headers.Set("X-Export-Filename", url.PathEscape(displayName))
