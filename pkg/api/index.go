...
// BMC Change: Starts
func (hs *HTTPServer) getFaviconLink(c *contextmodel.ReqContext) string {
	// Default favicon
	favicon := "public/img/fav32.png"

	if !external.FeatureFlagDashboardBranding.Enabled(c.Req, c.SignedInUser) {
		return favicon
	}

	epoch := time.Now().Unix()
	fileKey := fmt.Sprintf("/contents/css/custom.css?time=%d", epoch)
	uscurl := setting.UcsEndpoint + fileKey
	method := "GET"
	hs.log.Info("Requesting custom css file",
		"url", uscurl,
		"method", method,
	)
	client := &http.Client{}
	req, _ := http.NewRequest(method, uscurl, nil)

	referer := c.Req.Header.Get("Referer")
	if referer != "" {
		// Parse URL safely
		if u, err := url.Parse(referer); err == nil {
			// Build clean referer: host + path (no scheme, no trailing slash)
			cleanReferer := u.Host + strings.TrimSuffix(u.Path, "/")
			hs.log.Info("Request triggered", "referer", cleanReferer)
			// Assign only host part to outgoing request
			req.Host = u.Host
		} else {
			hs.log.Warn("Failed to parse referer", "raw", referer)
			return favicon
		}
	}
	...
	if err != nil {
		hs.log.Error("Failed to send a request to ucs service",
			"error", err.Error(),
		)
		return favicon
	}
	...
	if res.StatusCode != 200 {
		hs.log.Error("Failed to fetch custom css file from ucs service",
	...
	if err != nil {
		hs.log.Error("Failed to read body response",
			"error", err.Error(),
		)
		return favicon
	}
	...
	// Regex to capture: --favicon-name: someValue;
	re := regexp.MustCompile(`--favicon-name\s*:\s*([^;]+);`)
